...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `CRABCLAW_WHATSAPP_APP_SECRET` environment variable
    #[serde(default)]
...
impl Config {
    ...
    pub fn apply_env_overrides(&mut self) {
        // API Key: CRABCLAW_API_KEY or API_KEY
        if let Ok(key) = std::env::var("CRABCLAW_API_KEY").or_else(|_| std::env::var("API_KEY")) {
            if !key.is_empty() {
                self.api_key = Some(key);
            }
        }

        // Provider: CRABCLAW_PROVIDER or PROVIDER
        if let Ok(provider) =
            std::env::var("CRABCLAW_PROVIDER").or_else(|_| std::env::var("PROVIDER"))
        {
        ...
        // Model: CRABCLAW_MODEL
        if let Ok(model) = std::env::var("CRABCLAW_MODEL") {
            if !model.is_empty() {
                self.default_model = Some(model);
            }
        }

        // Workspace directory: CRABCLAW_WORKSPACE
        if let Ok(workspace) = std::env::var("CRABCLAW_WORKSPACE") {
            if !workspace.is_empty() {
                self.workspace_dir = PathBuf::from(workspace);
            }
        }

        // Gateway port: CRABCLAW_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("CRABCLAW_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
        {
        ...
        // Gateway host: CRABCLAW_GATEWAY_HOST or HOST
        if let Ok(host) = std::env::var("CRABCLAW_GATEWAY_HOST").or_else(|_| std::env::var("HOST"))
        {
            if !host.is_empty() {
        ...
        // Temperature: CRABCLAW_TEMPERATURE
        if let Ok(temp_str) = std::env::var("CRABCLAW_TEMPERATURE") {
...
"#;
    ...
    fn env_override_api_key() {
        ...
        std::env::set_var("CRABCLAW_API_KEY", "sk-test-env-key");
    ...
    fn env_override_model() {
        ...
        std::env::remove_var("CRABCLAW_MODEL");
    }
    ...
    fn env_override_workspace() {
        ...
        std::env::remove_var("CRABCLAW_WORKSPACE");
    }
    ...
    fn env_override_empty_values_ignored() {
        ...
        std::env::set_var("CRABCLAW_PROVIDER", "");
        config.apply_env_overrides();
    ...
    fn env_override_gateway_port() {
        ...
        std::env::set_var("CRABCLAW_GATEWAY_PORT", "8080");
        config.apply_env_overrides();
    ...
    fn env_override_gateway_host() {
        ...
        std::env::set_var("CRABCLAW_GATEWAY_HOST", "0.0.0.0");
        config.apply_env_overrides();
    ...
    fn env_override_temperature_out_of_range_ignored() {
        ...
        std::env::set_var("CRABCLAW_TEMPERATURE", "3.0");
        config.apply_env_overrides();
        assert!(
